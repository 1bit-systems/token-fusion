...
def test_cross_modal_fusion():
    """Cross-modal token exchange (TokenFusion CVPR 2022 style)."""
    np.random.seed(42)
    primary = np.random.randn(10, 32).astype(np.float32)
    auxiliary = np.random.randn(10, 32).astype(np.float32)  # Same shape required

    config = MergingConfig()
    fuser = CrossModalFuser(config, exchange_threshold=0.5)
    ...
    print(f"    Exchanged: {result.metadata['total_exchanged']} tokens")
    assert result.merged_tokens.shape == (20, 32), "Should preserve both modalities"
    assert result.metadata["method"] == "threshold_exchange"
